...
def result_process_af(video_names, start_frames, cls_scores, anchors_xmin, anchors_xmax, cfg, bd_score=None, vid_preds=None):
    out_df = pd.DataFrame()
    out_df_cc = pd.DataFrame()

    vid_preds = vid_preds[0].detach().cpu().numpy()
    cc_pred = []
    for i in range(vid_preds.shape[0]):
        if vid_preds[i] < 0.1:
            cc_pred.append(i + 1)
    ...
    out_df['video_name'] = vid_name_df
    out_df_cc['video_name'] = vid_name_df
    ...
    out_df_cc['xmin'] = xmins_tmp
    out_df_cc['xmax'] = xmaxs_tmp

    # ========= no constrain ============
    scores_action = cls_scores[:, :, 1:]
    max_values = np.amax(scores_action, axis=2)
    conf_tmp = np.reshape(max_values, num_element)
    conf_tmp = np.where(conf_tmp < 1e-6, 0, conf_tmp)
    bd_score = bd_score.squeeze().detach().cpu().numpy()
    bd_score = np.sqrt(bd_score)
    out_df['score'] = conf_tmp
    conf_tmp = conf_tmp * bd_score
    out_df['conf'] = conf_tmp
    max_idxs = np.argmax(scores_action, axis=2)
    max_idxs = max_idxs + 1
    cate_idx_tmp = np.reshape(max_idxs, num_element)
    # cate_idx_tmp = max_idxs
    # Notice: convert index into category type
    class_real = cfg.DATASET.CLASS_IDX
    for i in range(len(cate_idx_tmp)):
        cate_idx_tmp[i] = class_real[int(cate_idx_tmp[i])]
    ...
    # ========= class constrain =========
    if len(cc_pred) > 0:
        cls_scores[:, :, cc_pred] = 0

    scores_action = cls_scores[:, :, 1:]
    max_values = np.amax(scores_action, axis=2)
    conf_tmp = np.reshape(max_values, num_element)
    conf_tmp = np.where(conf_tmp < 1e-6, 0, conf_tmp)
    out_df_cc['score'] = conf_tmp
    conf_tmp = conf_tmp * bd_score
    out_df_cc['conf'] = conf_tmp
    max_idxs = np.argmax(scores_action, axis=2)
    max_idxs = max_idxs + 1
    cate_idx_tmp = np.reshape(max_idxs, num_element)
    # cate_idx_tmp = max_idxs
    ...
    for i in range(len(cate_idx_tmp)):
        cate_idx_tmp[i] = class_real[int(cate_idx_tmp[i])]
    out_df_cc['cate_idx'] = cate_idx_tmp

    out_df = out_df[out_df['score'] > 0]
    out_df_cc = out_df_cc[out_df_cc['score'] > 0]

    out_df = out_df[cfg.TEST.OUTDF_COLUMNS_AB]
    out_df_cc = out_df_cc[cfg.TEST.OUTDF_COLUMNS_AB]
    return out_df, out_df_cc
